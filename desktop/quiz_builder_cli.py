...
if getattr(sys, 'frozen', False):
    ...
    if not os.path.isdir(PYTHON_SCRIPTS_DIR):
        PYTHON_SCRIPTS_DIR = os.path.join(MEI_DIR, 'desktop', 'python_scripts')
        if not os.path.isdir(PYTHON_SCRIPTS_DIR):
...
REPO_ROOT = os.path.dirname(CLI_ROOT) if os.path.basename(CLI_ROOT) in ['desktop', 'python_app', 'cli-legacy'] else CLI_ROOT
...
def run_script(script_name, args):
    ...
    candidate_paths = [
        ...
        os.path.join(MEI_DIR, 'desktop', 'python_scripts', script_name),
