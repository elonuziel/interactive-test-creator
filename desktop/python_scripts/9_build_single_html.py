...
REPO_ROOT = os.path.dirname(APP_DIR) if os.path.basename(APP_DIR) in ['desktop', 'python_app', 'cli-legacy'] else APP_DIR
...
def resolve_web_asset(filename):
    ...
    if frozen_base:
        candidates.extend([
            os.path.join(frozen_base, 'desktop', 'web', filename),
